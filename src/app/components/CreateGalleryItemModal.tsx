  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setFormData(prev => ({
        ...
        endDate: yearLater.toISOString().split('T')[0]
      }));
    }
  }, [isOpen]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (images.length > 0 && result.data?._id) {
          for (const image of images) {
            await GalleryService.uploadImage(token, result.data._id, image);
          }
        }

        // Upload videos if any (only for verified organizations)
        if (videos.length > 0 && result.data?._id) {
      ...
      <div className="bg-white rounded-lg w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        <div className="p-6">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold text-gray-800">Create Gallery Item</h2>
            <button
              onClick={onClose}
